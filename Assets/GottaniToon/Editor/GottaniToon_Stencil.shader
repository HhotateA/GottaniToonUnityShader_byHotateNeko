Shader "Hidden/HOTATE/GottaniToon_Stencil"
{
	Properties
	{
...
SubShader
	{
		...
		GrabPass{"_GrabPassTexture_Stencil"}
		Stencil
		{
			Ref [_Stencil]
			ReadMask [_ReadMask]
			...
			f_out frag (g2f input, fixed facing : VFACE)
			{
				...
				//* ベースカラー専用処理
					...
					float parallaxMask;
					{
						...
						float3 planePos = rayPos - planeNormal*_ParallaxDepth;
						float intersect = 999;
						{
				...
				float grabAlpha;
				{
					...
					grabCol.r = tex2Dproj(_GrabPassTexture_Stencil, grabUV + _ChromaticAberrationR).r;
					grabCol.g = tex2Dproj(_GrabPassTexture_Stencil, grabUV + _ChromaticAberrationG).g;
					grabCol.b = tex2Dproj(_GrabPassTexture_Stencil, grabUV + _ChromaticAberrationB).b;
		...
		#include "AutoLight.cginc"
			...
			float4 _ParallaxScroll;
				...
				//GrabPass
				sampler2D_float _GrabPassTexture_Stencil;
