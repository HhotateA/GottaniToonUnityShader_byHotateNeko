...
SubShader
	{
		Tags {"Queue"="Geometry"}
		GrabPass{"_GrabTransPassTexture"}
		Stencil
		{
			Ref [_Stencil]
			ReadMask [_ReadMask]
			...
			f_out frag (g2f input, fixed facing : VFACE)
			{
				...
				//* ベースカラー専用処理
					...
					float grabAlpha;
					{
						...
						grabCol.r = tex2Dproj(_GrabTransPassTexture, grabUV + _ChromaticAberrationR).r;
						grabCol.g = tex2Dproj(_GrabTransPassTexture, grabUV + _ChromaticAberrationG).g;
						grabCol.b = tex2Dproj(_GrabTransPassTexture, grabUV + _ChromaticAberrationB).b;
		...
		#include "AutoLight.cginc"
			...
			#ifdef IS_GRAB_PASS
				//GrabPass
				sampler2D_float _GrabTransPassTexture;
			#endif
				int _Alpha2Grab;
				float _GrabBlend;
				float _GrabMul;
